...
const PhaseModel = (() => {
  ...
  function compute(input) {
    const { poolLength, lapDistance, stroke, idealTime, maxUWFromWallM = 15 } = input;
    ...
    for (let i = 0; i < numLengths; i++) {
      ...
      const lp = _buildLengthPhases(
        ...
        budget, poolLength, absDistCursor, absTimeCursor, maxUWFromWallM
      );
   ...
   */
  function _buildLengthPhases(lenIdx, isFirst, isLast, direction, c, strokeKey, budget, poolLength, distOff, timeOff, maxUWFromWallM) {
    const phases  = [];
    let curDist   = distOff;
    let curTime   = timeOff;
    ...
    const startZone    = Math.min(c.startZone, poolLength * 0.45, maxUWFromWallM);
